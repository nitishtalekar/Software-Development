...
//   }
  if($selected==0 &&  $status==0){
   ...
   elseif (($selected==1 && $status==1) ||($selected==1 && $status==0) ) {
    ...
    if($status == 0){
      $query = "update applied_student set selected_notselected = 0 where company_id=$userid and student_grno = $student_grno";
      $results = $conn -> query($query);
    }else{
      $query = "update applied_student set selected_notselected = 1 where company_id=$userid and student_grno = $student_grno";
      $results = $conn -> query($query);
      // $query = "insert into selected_student(student_grno,company_id,date) values($student_grno,$userid,'$date')";
      // $results = $conn -> query($query);
    }
    if($results){
      header('Location: /college_project/TandP/applied_students.php');
    }
