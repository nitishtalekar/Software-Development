...
class PDF(FPDF, HTMLMixin):
	def header(self):
		self.image('media/RGIT.png', x = 25, h = 30)


# pdf.cell(w, h = 0, txt = '', border = 0, ln = 0, align = '', fill = False, link = '')
def createreport(df, imgpath, path2, dept):
	...
	top3 = df.sort_values(['SGPI', 'TOTAL'], axis = 0, ascending = False, inplace = False, kind = 'quicksort',
	...
	for i in df.columns[2:-3]:
		sub2 = df.sort_values(i, axis = 0, ascending = False, inplace = False, kind = 'quicksort').iloc[:2, :]
		for j in range(2):
			if j == 0:
	...
	for i in df.columns[2:-3]:
		app.append(sum(df[i].apply(lambda x: int(x) > 0)))
		passed.append(sum(df[i].apply(lambda x: int(x) > 40)))
		psp.append(round(passed[-1] * 100 / app[-1], 2))
		m1.append(sum(df[i].apply(lambda x: int(x) >= 40 and int(x) < 50)))
		m2.append(sum(df[i].apply(lambda x: int(x) >= 50 and int(x) < 60)))
		m3.append(sum(df[i].apply(lambda x: int(x) >= 60)))
	...
	pdf.image(imgpath[0], x = 0, h = 100)

	pdf.cell(0, ln = 1)
	pdf.set_font('Arial', 'B', 12)
	pdf.cell(0, 5, 'SGPI Statistical Analysis:', 0, 1, 'L')
	pdf.image(imgpath[1], x = 0, h = 100)
